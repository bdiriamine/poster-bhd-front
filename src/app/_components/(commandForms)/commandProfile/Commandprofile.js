                        ...
                        {command.panier.map((item, index) => (
                            <div key={index} className="border-2 border-gray-200 rounded-lg p-4 flex bg-white shadow-sm hover:shadow-md transition duration-300 ease-in-out">
                                <div className="flex space-x-2">
                                    {item.images && Array.isArray(item.images) && item.images.map((imageurl, imgIndex) => (
                                        <Image
                                            key={imgIndex}
                                            src={imageurl} // Assuming each entry in item.images is the image URL
                                            alt={`Product Image ${imgIndex + 1}`}
                                            width={150} // Adjust width
                                            height={150} // Adjust height
                                            className="w-24 h-24 object-cover rounded-lg shadow-md sm:w-32 sm:h-32 md:w-40 md:h-40 lg:w-48 lg:h-48" // Responsive classes
                                        />
                                    ))}
                                </div>
